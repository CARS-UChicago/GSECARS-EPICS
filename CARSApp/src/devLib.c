...
#ifndef ALLOW_MULTIPLE_HANDLERS
			  return S_dev_vectorInUse;
/*		
   In ppc (Motorola mv230x),
   having multiple handlers attached to the same vector is OK; 
   allowing this can break some drivers, because:
   if the same driver is used twice for two different
   cards, it is not safe to disconnect from a vector using
   only the function pointer as a key.
   The default implementation allows only one handler/vector.
*/
#endif
		}
		status = intConnect(
...
#else
	if(!ppcIsConnected(vectorNumber,pFunction)) return S_dev_vectorNotInUse;
#endif
	switch(intType){
	case intCPU:
	case intVME:
...
#else
	  status = ppcDisconnectVec(vectorNumber,pFunction);
	  if(status<0)  return S_dev_vxWorksVecInstlFail;
#endif
	  break;
	default:
		return S_dev_uknIntType;
