...
set EPICS_DISPLAY_PATH=..\..\..\dxp\dxpApp\op\adl;..\..\..\mca\mcaApp\op\adl;..\..\..\autosave\asApp\op\adl
REM Note that we must use the Windows "start" command or medm won't find X11 dlls
start medm -x -macro "P=13Saturn1:, D=dxp1, M=mca1" single_dxp_top.adl
REM Put Cygwin in the path so the EPICS application can find cygwin1.dll
PATH=c:\cygwin\bin
..\..\bin\cygwin-x86\CARSApp.exe st.cmd
