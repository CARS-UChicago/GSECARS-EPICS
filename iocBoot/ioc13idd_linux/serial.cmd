...
iocshLoad ../asynIPPortConfig.cmd "PORT=serial3,  IPADDR=$(TS):4003, IEOS=\\r,     OEOS=\\r\\n" # BNC 505;      9600,'N',1,8,'N'  PP A03
#iocshLoad ../asynIPPortConfig.cmd "PORT=serial4,  IPADDR=$(TS):4004, IEOS=\\r,     OEOS=\\r"    # MKS          19200,'E',1,8,'N'  PP A04 Controlled by 13-ID-A
...
# Serial 19 and 20 are IPG-YLR laser over Ethernet
drvAsynIPPortConfigure("serial19", "10.100.100.20:10001", 0, 0, 0) 
drvAsynIPPortConfigure("serial20", "10.100.100.21:10001", 0, 0, 0) 
#drvAsynIPPortConfigure("serial19", "gse-ipg-ylr4:10001", 0, 0, 0) 
#drvAsynIPPortConfigure("serial20", "gse-ipg-ylr5:10001", 0, 0, 0)
asynOctetSetInputEos("serial19",0,"\r")
asynOctetSetOutputEos("serial19",0,"\r")
asynOctetSetInputEos("serial20",0,"\r")
...
# BNC 505
dbLoadRecords("$(DELAYGEN)/db/BNC_505.db",              "P=$(P), PORT=serial3, R=BNC1:")
dbLoadRecords("$(DELAYGEN)/db/BNC_505_Pn.db",           "P=$(P), PORT=serial3, R=BNC1:, N=1")
dbLoadRecords("$(DELAYGEN)/db/BNC_505_Pn.db",           "P=$(P), PORT=serial3, R=BNC1:, N=2")
dbLoadRecords("$(DELAYGEN)/db/BNC_505_Pn.db",           "P=$(P), PORT=serial3, R=BNC1:, N=3")
dbLoadRecords("$(DELAYGEN)/db/BNC_505_Pn.db",           "P=$(P), PORT=serial3, R=BNC1:, N=4")
# Laser power controller database currently prevents clean reboot, and it is not currently used.  Comment out for now.
#dbLoadRecords("102$(CARS)/db/lpc.db",                      "P=$(P), PORT=serial4,  L=LPC1_, DAC=DAC1_2")
...
dbLoadRecords("$(CARS)/db/IPG_YLR_laser.db",            "P=$(P), PORT=serial19, R=Laser1")
dbLoadRecords("$(CARS)/db/IPG_YLR_laser.db",            "P=$(P), PORT=serial20, R=Laser2")
